create schema if not exists waibao_v3 collate utf8mb4_general_ci;
use waibao_v3;
...
create table if not exists log_order_goods
(
    id            bigint(12) unsigned auto_increment comment '自增ID'
        primary key,
...
create table if not exists log_seckill_goods
(
    id                 bigint(12) unsigned auto_increment comment '自增ID'
        primary key,
...
create table if not exists log_user_credit
(
...
create table if not exists mq_msg_compensation
(
...
create index mq_msg_compensation_status_index
    on mq_msg_compensation (status);

create index mq_msg_compensation_tags_index
    on mq_msg_compensation (tags);

create index mq_msg_compensation_topic_index
    on mq_msg_compensation (topic);

create table if not exists order_retailer
(
    order_id      varchar(255)                  not null comment '订单ID'
        primary key,
    goods_id      bigint(12) unsigned           not null comment '商品ID',
